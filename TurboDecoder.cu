...
int main(int argc, char* argv[])
{
	...
	BYTE (*LastStateDevice)[4];
	BYTE (*NextStateDevice)[4];
	char (*LastOutDevice)[4];
	char (*NextOutDevice)[4];
	double * yDevice;
	double * msgDevice;
	double * imsgDevice;
	BYTE * mhatDevice;
	...
	double * L_aDevice;
	double * L_allDevice;
	double (*gammaAlphaDevice)[4][4];
	double (*gammaBetaDevice)[4][4];
	double (*AlphaDevice)[4];
	double (*BetaDevice)[4];
